...
class XEP_0325(BasePlugin):
    ...
    def _handle_set_req(self, iq):
        ...
        # Fields - for control we need to find all in all devices, otherwise we
        # reject
        process_fields = []
            ...
            for f in iq['set']['datas']:
                for node in self.nodes:
                    if not self.nodes[node]["device"].has_control_field(f['name'], f._get_typename()):
                        ...
                        break
            process_fields = [
                (f['name'], f._get_typename(), f['value']) for f in iq['set']['datas']]

        if req_ok:
            session = self._new_session()
            self.sessions[session] = {
                "from": iq['from'], "to": iq['to'], "seqnr": iq['id']}
            ...
            if self.threaded:
                ...
                tr_req = Thread(
                    target=self._threaded_node_request, args=(session, process_fields))
                tr_req.start()
            ...
            for f in msg['set']['datas']:
                for node in self.nodes:
                    if not self.nodes[node]["device"].has_control_field(f['name'], f._get_typename()):
                        ...
                        break
            process_fields = [
                (f['name'], f._get_typename(), f['value']) for f in msg['set']['datas']]
    ...
    def _threaded_node_request(self, session, process_fields):
        ...
        for node in self.sessions[session]["node_list"]:
            ...
            timer.start()
            self.nodes[node]['device'].set_control_fields(
                process_fields, session=session, callback=self._device_set_command_callback)
    ...
    def _device_set_command_callback(self, session, nodeId, result, error_field=None, error_msg=None):
        ...
        if not session in self.sessions:
            # This can happend if a session was deleted, like in a timeout.
            # Just drop the data.
            return
    ...
    def set_request(self, from_jid, to_jid, callback, fields, nodeIds=None):
        ...
        self.sessions[seqnr] = {
            "from": iq['from'], "to": iq['to'], "callback": callback}
        iq.send(block=False)
    ...
    def _handle_set_response(self, iq):
        ...
        callback(from_jid=from_jid, result=result, nodeIds=nodeIds,
                 fields=fields, error_msg=error_msg)
